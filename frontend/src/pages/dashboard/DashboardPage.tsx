...
export function DashboardPage() {
  ...
  const [completedPickupsLoading, setCompletedPickupsLoading] = useState(false);
  ...
  useEffect(() => {
    if (user?.role === 'RECYCLER') {
      ...
      setCompletedPickupsLoading(true);
      ...
      // Fetch completed pickups that need processing and already processed/rejected ones
      apiService.getPickups()
        .then(res => {
          const allPickups = res.data.data || [];
          const relevantPickups = allPickups.filter(p => 
            p.status === 'completed' || p.status === 'processed' || p.status === 'rejected'
          );
          setCompletedPickups(relevantPickups);
        })
        .catch(() => setCompletedPickups([]))
        .finally(() => setCompletedPickupsLoading(false));
    }
  }, [user?.role]);
  ...
  const getRecyclerStats = () => {
    // Count processed and rejected from all completed pickups (not just facility-assigned)
    const processed = completedPickups.filter(p => p.status === 'processed').length;
    const rejected = completedPickups.filter(p => p.status === 'rejected').length;
    
    // Calculate facility-related stats
    const facilityIds = facilities.map(f => f._id);
    const facilityPickups = userPickups.filter(p => p.facilityId && facilityIds.includes(p.facilityId));
    const totalWeight = facilityPickups.reduce((sum, p) => sum + (p.receivedWeightKg || p.actualWeightKg || p.estimatedWeightKg), 0);
    ...
    return [
      ...
      { name: 'Processed', value: String(processed), icon: CheckCircle, color: 'text-green-600', bgColor: 'bg-green-50 dark:bg-green-900/20' },
      ...
      <div className="space-y-8">
        ...
        {user?.role === 'RECYCLER' && (
          <div className="space-y-6">
            ...
            {/* Pickup Processing Center */}
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-3">
                <Package className="h-7 w-7 text-orange-600" />
                Pickup Processing Center
                {completedPickups.length > 0 && (
                  <span className="bg-orange-100 text-orange-800 dark:bg-orange-900/20 dark:text-orange-300 text-sm font-medium px-2.5 py-0.5 rounded-full">
                    {completedPickups.length}
                  </span>
                )}
              </h2>
              {completedPickupsLoading ? (
                <div className="text-gray-600 dark:text-gray-400 text-center py-8">Loading completed pickups...</div>
              ) : completedPickups.length === 0 ? (
                <div className="text-gray-600 dark:text-gray-400 text-center py-8">
                  <CheckCircle className="h-12 w-12 mx-auto mb-4 opacity-50" />
                  <p className="text-lg">No pickups to process or review</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 gap-4">
                  {completedPickups.map(pickup => (
                    <div key={pickup._id} className="bg-gray-50 dark:bg-gray-700 rounded-xl p-6 border border-gray-200 dark:border-gray-600">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            <h3 className="font-bold text-gray-900 dark:text-white text-lg">
                              {pickup.wasteType.replace('_', ' ').toUpperCase()}
                            </h3>
                            <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                              ...
                              {pickup.status === 'completed' ? 'Awaiting Processing' : pickup.status.charAt(0).toUpperCase() + pickup.status.slice(1)}
                            </span>
                          </div>
                          <p className="text-gray-600 dark:text-gray-400 text-sm mb-2">{pickup.address}</p>
                          <div className="grid grid-cols-2 gap-4 text-sm">
                            <div>
                              <span className="text-gray-500 dark:text-gray-400">Weight:</span>
                              <span className="font-medium text-gray-900 dark:text-white ml-1">
                                {pickup.actualWeightKg || pickup.estimatedWeightKg} kg
                              </span>
                            </div>
                            <div>
                          ...
                          {pickup.contaminationScore !== undefined && (
                            <div className="mt-2">
                              <span className="text-gray-500 dark:text-gray-400 text-sm">Contamination:</span>
                              <span className={`ml-1 text-sm px-2 py-1 rounded-full ${pickup.contaminationScore > 70 ? 'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-300' :
                          ...
                          {pickup.status === 'completed' && (
                            <>
                              <button
                                onClick={async () => {
                                  try {
                                    await apiService.receivePickup(pickup._id, { receivedWeightKg: pickup.actualWeightKg || pickup.estimatedWeightKg });
