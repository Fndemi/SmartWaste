...
export function CreatePickupPage() {
  ...
  const onSubmit = async (data: CreatePickupForm) => {
    ...
    toast.success('Pickup request submitted successfully! You will receive a notification once processing is complete.');
    ...
    // Make the API call in background after showing success message
    try {
      await apiService.createPickup(payload, selectedImage);
      // eslint-disable-next-line no-console
      console.log('[ui] Background pickup creation completed successfully');
    } catch (error: unknown) {
      const err = error as any;
      // eslint-disable-next-line no-console
      console.error('[ui] Background pickup creation failed', {
        message: err?.message,
        status: err?.response?.status,
        response: err?.response?.data,
      });
      // Optionally show a subtle notification that processing failed
      // but don't disrupt the user experience since they already navigated away
    }
  };
  return (
    <DashboardLayout>
