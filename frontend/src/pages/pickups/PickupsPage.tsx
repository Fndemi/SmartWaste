    ...
    try {
      ...
      if (user?.role === 'DRIVER' && showAvailable) {
        ...
        if (user?.role === 'DRIVER') {
          ...
          // Show pickups assigned to facilities they manage AND all completed/processed/rejected pickups
          pickupsData = pickupsData.filter(p => 
            (p.facilityId && p.status !== 'pending') || // Pickups assigned to facilities
            p.status === 'completed' || // Completed pickups awaiting processing
            p.status === 'processed' || // Already processed pickups
            p.status === 'rejected' // Rejected pickups
          );
        }
          ...
          <div>
            ...
            <p className="text-ink-700 dark:text-ink-300 mt-1">
              {user?.role === 'DRIVER' && showAvailable 
                ...
                : user?.role === 'RECYCLER'
                ? 'View and manage pickups: completed (awaiting processing), processed, rejected, and facility-assigned'
                  ...
                  <div key={pickup._id} className="p-4 sm:p-6 hover:bg-ink-50 dark:hover:bg-ink-700/50 transition-colors duration-200">
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-2">
                        ...
                        {user?.role === 'RECYCLER' && pickup.status === 'completed' && (
                          <Button
                            size="sm"
                            onClick={async () => {
                              try {
