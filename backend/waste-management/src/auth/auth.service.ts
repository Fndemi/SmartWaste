...
export class AuthService {
  ...
  async register(dto: RegisterDto) {
    ...
    try {
      ...
      console.log(
        '🔑 Creating user with hashed password and verification token',
      );
      ...
      try {
        ...
        const verificationUrl = `${backendUrl}/users/verify-email?token=${emailVerificationToken}`;
        ...
        if (user.email) {
          ...
          console.error(
            '❌ Cannot send verification email: User email is undefined',
          );
        }
      } catch (emailError) {
  ...
  async login(dto: { email: string; password: string }) {
    ...
    if (!user.isEmailVerified) {
      ...
      if (
        ...
        const backendUrl = this.config.get('BACKEND_URL', 'http://localhost:3500');
        if (user.email) {
  ...
  async forgotPassword(
    ...
    if (!user || !user.email) {
      console.log(
        '⚠️  Email not found (not revealing this to user for security)',
      );
