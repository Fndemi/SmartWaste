...
const LABELS = [
  { min: 0, max: 0.2, label: 'Clean' as const },
  { min: 0.2, max: 0.4, label: 'Low' as const },
  { min: 0.4, max: 0.7, label: 'Moderate' as const },
  { min: 0.7, max: 1.0, label: 'High' as const },
];
...
export class ContaminationClient {
  ...
  private readonly alertThreshold: number; // 0..1 scale
  ...
  constructor(
    ...
    // Convert threshold from 1-10 scale to 0-1 scale for backward compatibility
    const rawThreshold = Number(
      this.config.get<string>('CONTAMINATION_ALERT_THRESHOLD') ?? 6,
    );
    this.alertThreshold = rawThreshold > 1 ? (rawThreshold - 1) / 9 : rawThreshold;
...
Return ONLY JSON matching the schema.`;
    ...
    const rawScore = this.clampInt(parsed.score, 1, 10);
    const score = this.normalizeScore(rawScore); // Convert 1-10 to 0-1
  ...
  /** Accepts 0–1, 0–100, or 1–10 and normalizes to 0–1 decimal */
  private normalizeScore(raw: unknown): number {
    ...
    if (n <= 1) {
      ...
      // assume 1..10 scale
      normalized = (n - 1) / 9; // Convert 1-10 to 0-1
    }
