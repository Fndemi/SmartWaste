...
export class UsersService {
  ...
  async create(data: Partial<IUser>): Promise<IUser> {
    ...
    try {
      ...
      if (error.code === 11000) {
        console.error(
          '❌ [UsersService] Duplicate email detected:',
          data.email,
        );
  ...
  async update(id: string, updateData: Partial<UserType>) {
    console.log(
      `\n🔄 [UsersService] Updating user ${id} with data:`,
    ...
    try {
      ...
      if (!updatedUser) {
        console.warn(
          `⚠️  [UsersService] Unable to fetch updated user ${id}, but update was successful`,
        );
  ...
  async findByEmailWithPassword(email: string): Promise<IUser | null> {
    if (!email) {
      console.error(
        '❌ [UsersService] No email provided to findByEmailWithPassword',
      );
      return null;
    }
    ...
    console.log(
      `🔍 [UsersService] findByEmailWithPassword: Searching for email: ${normalizedEmail}`,
    );
    ...
    try {
      ...
      if (user) {
        ...
        console.log(
          `❌ [UsersService] No user found with email: ${normalizedEmail}`,
        );
      }
  ...
  async updateProfile(
    ...
    try {
      ...
      if (!user) {
        ...
        console.log(
          '🔍 [updateProfile] All users in database:',
          allUsers.map((u) => ({ id: u._id.toString(), email: u.email })),
        );
      ...
      if (updateData.email && updateData.email !== user.email) {
        console.log(
          `📧 [updateProfile] Email change requested from ${user.email} to ${updateData.email}`,
        );

        const emailExists = await this.existsByEmail(updateData.email);
        if (emailExists) {
          console.error(
            `❌ [updateProfile] Email already in use: ${updateData.email}`,
          );
          throw new BadRequestException('Email already in use');
        }
        ...
        update.emailVerificationExpires = new Date(
          Date.now() + 24 * 60 * 60 * 1000,
        );
        console.log(
          `🔑 [updateProfile] Generated new email verification token for ${update.email}`,
        );
      }
      ...
      // Perform the update
      console.log(
        `🔄 [updateProfile] Updating user ${userId} with data:`,
        update,
      );
      ...
      if (!updatedUser) {
        console.error(
          `❌ [updateProfile] Failed to update user ${userId} - no user returned`,
        );
        throw new InternalServerErrorException('Failed to update user');
      }
      ...
      if (update.emailVerificationToken && updatedUser.email) {
        try {
          ...
          console.log(
            `📧 [updateProfile] Sent verification email to ${updatedUser.email}`,
          );
        } catch (emailError) {
          console.error(
            `❌ [updateProfile] Failed to send verification email:`,
            emailError,
          );
  ...
  async changePassword(
    ...
    try {
      ...
      if (!user.passwordHash) {
        console.error(
          `❌ [changePassword] No password set for user: ${user.email}`,
        );
        throw new BadRequestException(
          'No password set for this account. Please use the password reset flow.',
        );
      }
      ...
      if (!isPasswordValid) {
        console.error(
          `❌ [changePassword] Invalid current password for user: ${user.email}`,
        );
      ...
      console.log(
        `✅ [changePassword] Password updated successfully for user: ${user.email}`,
      );
    } catch (error) {
      console.error(
        `❌ [changePassword] Error changing password for user ${userId}:`,
        error,
      );
  ...
  async resendVerificationEmail(
    ...
    try {
      console.log(
        `🔄 [resendVerificationEmail] Processing request for email: ${email}`,
      );
      ...
      if (!user) {
        console.log(
          `⚠️  [resendVerificationEmail] Email not found: ${normalizedEmail}`,
        );
      ...
      if (user.isEmailVerified) {
        console.log(
          `ℹ️  [resendVerificationEmail] Email already verified: ${normalizedEmail}`,
        );
        return {
          status: 200,
      ...
      const verificationUrl = `${backendUrl}/users/verify-email?token=${verificationToken}`;
      ...
      console.log(
        `✅ [resendVerificationEmail] Verification email sent to: ${user.email}`,
      );
